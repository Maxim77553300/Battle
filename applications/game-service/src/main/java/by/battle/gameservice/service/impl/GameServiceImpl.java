...
public class GameServiceImpl implements GameService {
    ...
    private Game findGame(Move move) {
        return gameRepository.findById(move.getGame().getId())
                .orElseThrow(() -> new ResourceNotFoundException(move.getGame().getName()));
    }

    private void isGameHasNotStatusFinished(Game game) {
        if (game.getStatus().equals(GameStatus.FINISHED)) {
            throw new ResourceNotFoundException(game.getName() + " finished");
        }
    }
    ...
    private void isFiguresAreDifferent(Game game) {
        if (game.getPlayerFigures().get(0).getFigure().equals(game.getPlayerFigures().get(1).getFigure())) {
            throw new ObjectAlreadyExistsException(game.getPlayerFigures().get(0).getFigure().toString());
        }
    }
